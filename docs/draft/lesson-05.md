...
- 第 6 课按 v4 安排 80 分钟教学 + 15 分钟小测 + 5 分钟缓冲；[新版第 6 课](lesson-06.md)已给出五表与迁移交接，标签去重等变化从该阶段起登记，不提前改动本课 M1 规则。
...
- **跨课核对**：第 5 课继续第 4 课的 SQLite、五字段、四字段 JSON 错误、单 status 探针及服务层提交，HTML 仅增加表现适配。第 6 课底稿现已按 v4 重写，迁移清单、批准差异及目标数据库版本的待验收项见其制作状态。临时核验脚本不是已交付的学生自检器或教师并发工具。
