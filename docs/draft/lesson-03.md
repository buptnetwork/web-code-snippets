...
本次课 100 分钟，留 5 分钟缓冲。前两次课站在**链路整体**看问题；本次课把第 1 次课链路图里的一个环节——「路由与参数解析」——**放大来看**，把框架的"魔法"拆成可以打印出来、可以下断点、可以逐字段验证的东西。
...
### 作业一：六个端点的 In/Out schema（主线作业）

把项目收敛成下面六个端点，**每一个都必须有明确的出口模型，禁止返回裸 dict**：
...
| 4 | `PATCH /questions/{qid}` | `QuestionUpdate`（字段全可选） | `QuestionOut` | 200 |
| 5 | `POST /questions/{qid}/answers` | `AnswerCreate` | `AnswerOut` | **201** |
| 6 | `GET /healthz` | — | `HealthOut` | 200 / 503 |
...
**至少 10 行**，其中必须包含 ≥1 个"结果与你的预期不一致"的案例并说明原因。
