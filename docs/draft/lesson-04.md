...
本次课 95 分钟，留 5 分钟缓冲。**本次课的定位是：在不破坏契约的前提下完成一次可控重构。请严格按这个顺序制作，不要提前给出结论。**

具体来说：课件中**不允许在单元 2 结束之前出现"依赖注入""分层架构"这两个词**，也不要出现目录结构图。学生必须先经历"改一个小需求要动多个地方"，才有资格听"所以我们把它抽出来"。如果颠倒顺序，这节课就退化成一堂架构名词介绍课——那正是本课程要避免的教法。

**本次课的主线**：第 3 次课把"数据的进出"收拢到了两道闸门。本次课收拢的是**代码本身的位置**——同一件事只在一个地方写，并且那个地方是可以被指认的。

**课堂与课后边界**：
- **课堂完成**：重构演示与错误契约（单元 2、5、6）
- **课后自学**：依赖工厂（单元 4 第五种用法）、完整依赖树打印、洋葱模型详细推导、认证取舍、C 档卡
...
1. **需求变更的痛感演示（单元 2）**。难点在于学生会觉得"改多个地方也就多花五分钟"。必须补上第二层：**改多个地方意味着漏改 1 个地方不会有任何提示**。请把"漏改一处"的后果现场演示出来。
...
**事务时序说明**：
- 当前 FastAPI 版本中，默认请求作用域的依赖退出阶段在响应发送后
- 若要求提交成功后才能回应，应使用函数作用域或显式保证
- 本底稿单元 4 的 yield 依赖演示需注意此时序
...
### 作业一：完成一次可控重构（主线，交付重构后的项目）

对第 3 次课的核心端点做重构，确保原有行为不退化，并满足以下要求：
...
> **注意**：重构的目标是"在不破坏契约的前提下完成可控重构"，不是"消除全部重复"。如果某些重复是合理的（例如不同端点的特殊处理），请在提交说明中解释。

> **路由函数行数**：不强制要求不超过 10 行。如果路由函数较长，请在提交说明中解释原因。
...
> **注意**：如果 AI 给出的方案合理，请说明理由。评分重点不是"AI 必须给错"，而是能不能引用判据说明理由，能不能预判延迟出现的代价。
