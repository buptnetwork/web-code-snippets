> **历史快照 · 2026-09-17 通用化改造前**：下方保留旧版流程全文、原节号与当时的评审和实测结论，不再作为现行操作规范。
> 现行入口：[通用制作流程](../authoring-workflow.md)、[教师底稿指南](../lesson-draft-guide.md)、[Slidev 实施指南](../slidev-authoring-guide.md)、[当前课程约定](../course-conventions.md)。
> 原文中的页数、行号、版本、完成状态及“已验证”均指历史现场，不代表当前工程状态或跨课程验证结果。除修复相对链接外，不重写原文结论；原文对配套文档节号的引用应按当时版本理解。
...
本项目用 Slidev（Markdown + Vue + UnoCSS）制作《数据结构》课件，规模预期为十余章、数十次课、数百页，需长期维护与逐年迭代。
...
外置的收益远超"省事"：代码可编译、可写测试验证算法正确性、可直接发给学生、多页引用同一份实现（改一处全部生效）、md 显著变短从而更适合 AI 就地编辑。对"代码即内容"的《数据结构》课程，这是硬规定。
